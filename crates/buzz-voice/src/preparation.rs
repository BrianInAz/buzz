...
/// Clamp and shape one model-sized unit within a playback chunk.
///
/// Model-aware splitting may divide one sentence-first playback chunk into
/// several units. Only the outer boundaries receive padding and fade-out so
/// token-only boundaries remain sample-contiguous.
pub fn shape_tts_model_unit(
    samples: Vec<f32>,
    ...
    let lead_in = usize::from(starts_playback_chunk) * LEAD_IN_SAMPLES;
    let trailing_silence = usize::from(ends_playback_chunk) * TRAILING_SILENCE_SAMPLES;
    let mut shaped = Vec::with_capacity(lead_in + audio.len() + trailing_silence);
    shaped.resize(lead_in, 0.0);
    shaped.extend(audio);
    shaped.resize(shaped.len() + trailing_silence, 0.0);
    shaped
}

/// Clamp and shape a complete Pocket playback chunk.
///
...
pub fn shape_tts_chunk(samples: Vec<f32>) -> Vec<f32> {
    shape_tts_model_unit(samples, true, true)
}
...
mod tests {
    ...
    #[test]
    fn model_units_are_contiguous_inside_a_playback_chunk() {
        let first = shape_tts_model_unit(vec![0.5; 400], true, false);
        let last = shape_tts_model_unit(vec![0.5; 400], false, true);

        assert_eq!(first.len(), LEAD_IN_SAMPLES + 400);
        assert!(first[..LEAD_IN_SAMPLES].iter().all(|sample| *sample == 0.0));
        assert_eq!(first.last(), Some(&0.5));

        assert_eq!(last.len(), 400 + TRAILING_SILENCE_SAMPLES);
        assert_eq!(last[0], 0.5);
        assert_eq!(last[399], 0.0);
        assert!(last[400..].iter().all(|sample| *sample == 0.0));
    }
}
