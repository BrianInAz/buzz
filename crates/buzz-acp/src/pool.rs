    ...
    CountFut: std::future::Future<Output = Result<serde_json::Value, crate::relay::RelayError>>,
{
    ...
    let context = fetch_with_retry(|| async {
        ...
        .await
        {
            Ok(Ok(json)) => parse_nostr_thread_response(json, root_event_id, limit, &agent_pubkey),
            Ok(Err(e)) => {
                tracing::warn!(
                    channel_id = %channel_id,
                    root = root_event_id,
    ...
    if matches!(
        ...
        if let Some(total) = fetch_thread_total(channel_id, &replies_count_filter, &count).await {
    ...
    CountFut: std::future::Future<Output = Result<serde_json::Value, crate::relay::RelayError>>,
{
    fetch_with_retry(|| async {
        let replies_count =
            match timeout(CONTEXT_FETCH_TIMEOUT, count(vec![replies_filter.clone()])).await {
                Ok(Ok(json)) => json.get("count").and_then(|v| v.as_u64())?,
                Ok(Err(e)) => {
                    tracing::debug!(
                        channel_id = %channel_id,
                        "thread context count failed: {e} — will retry"
                    );
                    return None;
                }
                Err(_) => {
                    tracing::debug!(
                        channel_id = %channel_id,
                        "thread context count timed out — will retry"
                    );
                    return None;
                }
            };

        // +1 for the root event fetched by ID alongside matching replies.
        Some(replies_count as usize + 1)
    })
    .await
}
...
fn parse_nostr_thread_response(
    ...
    let fetched_total = reply_msgs.len() + usize::from(root_msg.is_some());
    ...
    if truncated {
        ...
        reply_msgs.sort_by_key(|(ts, _, _)| Reverse(*ts));
        reply_msgs.truncate(limit as usize);
    }

    if let Some(agent_reply) = newest_agent_reply {
        let agent_reply_already_displayed = reply_msgs.iter().any(|(ts, is_agent, msg)| {
            *ts == agent_reply.0
                && *is_agent == agent_reply.1
                && msg.pubkey.eq_ignore_ascii_case(&agent_reply.2.pubkey)
                && msg.content == agent_reply.2.content
        });
        if !agent_reply_already_displayed {
            reply_msgs.sort_by_key(|(ts, _, _)| *ts);
