...
const overrides = new Map([
  ...
  // +66 (1863 -> 1929): OpenRouter provider requirements — OPENROUTER_MODEL
  // fallback + OPENROUTER_API_KEY match arms in buzz_agent_requirements and
  // goose_requirements, plus the three readiness tests that pin them.
  ["src-tauri/src/managed_agents/readiness.rs", 1929],
  ...
  // +42 (1152 -> 1194): OpenRouter discovery call sites + module delegation
  // (bulk logic lives in agent_models_openrouter.rs), OpenRouter test imports,
  // and draft_agent_model_discovery_env extracted so discover_agent_models's
  // provider/env derivation is a tested pure function (Thufir T4 finding —
  // unsaved-agent discovery had no seam to exercise).
  ["src-tauri/src/commands/agent_models.rs", 1194],
